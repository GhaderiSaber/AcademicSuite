...
ACADEMIC_DEFENSE_ROLE_LAYOUTS = {
    "cover": "defense_cover",
    "title": "defense_cover",
    "committee": "defense_committee",
    "board": "defense_committee",
    "problem": "defense_problem",
    "problem_statement": "defense_problem",
    "problem_funnel": "defense_problem",
    "gap": "defense_gap",
    "research_gap": "defense_gap",
    "gap_matrix": "defense_gap",
    "callout": "defense_callout",
    "big_idea": "defense_callout",
    "divider": "defense_divider",
    "section_divider": "defense_divider",
    "framework": "defense_framework",
    "conceptual_model": "defense_framework",
    "model": "defense_framework",
    "methodology": "defense_methodology",
    "research_design": "defense_methodology",
    "method": "defense_methodology",
    "sample_flow": "defense_methodology",
    "instruments": "defense_instruments",
    "instrument_matrix": "defense_instruments",
    "scales": "defense_instruments",
    "intervention": "defense_intervention",
    "intervention_timeline": "defense_intervention",
    "protocol": "defense_intervention",
    "descriptive": "defense_descriptive",
    "descriptive_stats": "defense_descriptive",
    "split": "defense_split",
    "two_column": "defense_split",
    "comparison": "defense_split",
    "spotlight": "defense_spotlight",
    "result_spotlight": "defense_spotlight",
    "table": "defense_table",
    "stat_table": "defense_table",
    "ancova": "defense_table",
    "matrix": "defense_matrix",
    "hypothesis_matrix": "defense_matrix",
    "hypothesis": "defense_matrix",
    "discussion": "defense_discussion",
    "discussion_mechanism": "defense_discussion",
    "mechanism": "defense_discussion",
    "implications": "defense_implications",
    "limitations": "defense_limitations",
    "recommendations": "defense_limitations",
    "closing": "defense_closing",
    "cta": "defense_closing",
    "cta_close": "defense_closing",
    "hook": "defense_cover",
    "baseline": "defense_problem",
    "solution": "defense_framework",
    "evidence": "defense_table",
    "proof": "defense_matrix",
    "metrics": "defense_spotlight",
    "workflow": "defense_methodology",
    "timeline": "defense_intervention",
    "discovery": "defense_descriptive",
    "definition": "defense_problem",
    "feature": "defense_instruments",
    "features": "defense_instruments",
}
...
def validate_brief_data(brief: Any) -> list[str]:
    ...
    if not isinstance(deck, dict):
        ...
        if "page_count" in deck:
            _ensure_integer(deck["page_count"], "brief.deck.page_count", errors, minimum=5, maximum=100)
    ...
    if not isinstance(narrative, dict):
        ...
        if not isinstance(narrative.get("slides"), list) or len(slides) < 5:
            ...
            for index, slide in enumerate(slides, start=1):
                ...
                for key in ("claim", "explanation", "title_emphasis", "visual_intent", "preferred_layout_family", "notes", "speaker_note"):
...
def load_brief(path: str | Path) -> dict[str, Any]:
    ...
    if isinstance(data, dict) and "narrative" not in data and ("slides" in data or "meta" in data or "hypotheses" in data):
        from academic_brief_adapter import adapt_academic_payload_to_brief
        data = adapt_academic_payload_to_brief(data)
...
def _layout_map_for_preset(preset: str) -> dict[str, str]:
    normalized = _normalize_preset_name(preset)
    if normalized in {"academic defense", "academic-defense", "academic navy"}:
        return ACADEMIC_DEFENSE_ROLE_LAYOUTS
...
def _avoid_academic_defense_long_layout_runs(
    spec: dict[str, Any],
    layout_id: str,
    ...
    if not previous_layouts:
        return layout_id
    if previous_layouts[-1] != layout_id:
        return layout_id

    alternatives_map = {
        "defense_table": ["defense_matrix", "defense_spotlight", "defense_split"],
        "defense_split": ["defense_spotlight", "defense_matrix", "defense_callout"],
        "defense_matrix": ["defense_table", "defense_spotlight", "defense_split"],
        "defense_spotlight": ["defense_table", "defense_matrix", "defense_callout"],
        "defense_problem": ["defense_gap", "defense_callout"],
        "defense_gap": ["defense_problem", "defense_framework"],
        "defense_callout": ["defense_split", "defense_matrix"],
        "defense_framework": ["defense_methodology", "defense_split"],
        "defense_methodology": ["defense_instruments", "defense_framework"],
        "defense_instruments": ["defense_table", "defense_methodology"],
        "defense_intervention": ["defense_methodology", "defense_split"],
        "defense_descriptive": ["defense_table", "defense_spotlight"],
        "defense_discussion": ["defense_split", "defense_implications"],
        "defense_implications": ["defense_limitations", "defense_discussion"],
        "defense_limitations": ["defense_implications", "defense_discussion"],
    }
    candidates = alternatives_map.get(layout_id, [])
    for alt in candidates:
        if alt in layout_cycle and alt != previous_layouts[-1]:
            return alt
    for candidate in layout_cycle:
        if candidate != layout_id and candidate != previous_layouts[-1]:
            return candidate
    return layout_id
...
def build_slide_spec(brief: dict[str, Any], packet: dict[str, Any] | None = None) -> list[dict[str, Any]]:
    ...
    for index, slide in enumerate(brief["narrative"]["slides"], start=1):
        ...
        if slide_layout and (slide_layout in layout_cycle or slide_layout in allowed_layouts):
            layout_id = slide_layout
        else:
        ...
        spec = {
            ...
            "speaker_note": slide.get("speaker_note") or slide.get("notes") or f"{role}: {explanation}",
            "visual": visual_intent,
            "visual_intent": visual_intent,
            "preferred_layout_family": preferred_layout_family,
            "chart_policy": chart_policy,
            "quality_tier": quality_tier,
            "academic_data": slide.get("academic_data"),
        }
        ...
        elif normalized_preset in {"academic defense", "academic-defense", "academic navy"}:
            spec["layout_id"] = _avoid_academic_defense_long_layout_runs(spec, spec["layout_id"], previous_layouts, layout_cycle)
        else:
...
def _render_academic_defense_slide(spec: dict[str, Any], total: int, *, role_index: int) -> str:
    slide_number = spec["slide_number"]
    role = spec["role"]
    layout_id = spec["layout_id"]
    layout_class = re.sub(r"[^a-zA-Z0-9_-]+", "-", str(layout_id)).strip("-") or "default"
    layout_class = f"layout-{layout_class}"
    notes = _escape(spec.get("speaker_note") or "")
    title = _escape(spec.get("title") or "")
    key_point = _escape(spec.get("key_point") or "")
    claim = _escape(spec.get("claim") or "")
    items = spec.get("supporting_items") or []
    evidence = spec.get("evidence_items") or []
    academic_data = spec.get("academic_data") or {}
    counter_html = f'<div class="slide-counter">{slide_number} / {total}</div>'

    # 1. Cover
    if layout_id == "defense_cover" or role in {"cover", "title"} or role_index == 0:
        meta = academic_data.get("meta") or {}
        author = _escape(meta.get("author") or academic_data.get("author") or "پژوهشگر: صابر قادری")
        supervisor = _escape(meta.get("supervisor") or academic_data.get("supervisor") or "استاد راهنما: دکتر نام استاد")
        advisor = _escape(meta.get("advisor") or academic_data.get("advisor") or "استاد مشاور: دکتر نام مشاور")
        university = _escape(meta.get("university") or academic_data.get("university") or "دانشکده روان‌شناسی و علوم تربیتی")
        sub_text = key_point or claim or "جلسه دفاع از رساله دکتری تخصصی / پایان‌نامه کارشناسی ارشد"
        return f"""
    <section class="slide ad-slide-cover {layout_class}" id="slide-{slide_number}" data-notes="{notes}" aria-label="{_escape(role)}" data-export-role="{_escape(role)}">
        <div class="ad-hero-container">
            <span class="ad-eyebrow">جلسه دفاع از رساله دکتری تخصصی / پایان‌نامه کارشناسی ارشد</span>
            <h1 class="ad-title ad-hero-title">{title}</h1>
            <p class="ad-body ad-hero-sub">{sub_text}</p>
            <div class="ad-meta-grid">
                <div class="ad-meta-card"><span class="ad-meta-label">پژوهشگر</span><span class="ad-meta-val">{author}</span></div>
                <div class="ad-meta-card"><span class="ad-meta-label">استاد راهنما</span><span class="ad-meta-val">{supervisor}</span></div>
                <div class="ad-meta-card"><span class="ad-meta-label">استاد مشاور</span><span class="ad-meta-val">{advisor}</span></div>
                <div class="ad-meta-card"><span class="ad-meta-label">دانشگاه / موسسه</span><span class="ad-meta-val">{university}</span></div>
            </div>
        </div>
        {counter_html}
    </section>""".strip()
    ...
    # 2. Committee
    if layout_id == "defense_committee" or role in {"committee", "board"}:
        members = academic_data.get("members") or [
            {"role": "استاد راهنما", "name": "دکتر راهنما (استاد تمام)"},
            {"role": "استاد مشاور", "name": "دکتر مشاور (دانشیار)"},
            {"role": "داور خارجی", "name": "دکتر داور خارجی (دانشیار)"},
            {"role": "داور داخلی", "name": "دکتر داور داخلی (استادیار)"},
        ]
        cards_html = "".join(
            f'<div class="ad-comm-card"><div class="ad-comm-role">{_escape(m.get("role", ""))}</div><div class="ad-comm-name">{_escape(m.get("name", ""))}</div></div>'
            for m in members
        )
        return f"""
    <section class="slide ad-slide-committee {layout_class}" id="slide-{slide_number}" data-notes="{notes}" aria-label="{_escape(role)}" data-export-role="{_escape(role)}">
        <div class="ad-header">
            <span class="ad-eyebrow">هیئت محترم داوران و هیئت نظارت دفاع</span>
            <h2 class="ad-title">{title}</h2>
            <p class="ad-sub">{key_point}</p>
        </div>
        <div class="ad-committee-grid">
            {cards_html}
        </div>
        {counter_html}
    </section>""".strip()
    ...
    # 3. Problem Funnel
    if layout_id == "defense_problem" or role in {"problem", "problem_statement", "problem_funnel"}:
        stages = academic_data.get("stages") or [
            {"step": "۰۱", "title": "شیوع و گستردگی اختلال در جامعه آماری", "desc": items[0] if len(items) > 0 else "بررسی ابعاد همه‌گیری‌شناسی و بار تحمیل‌شده به سامانه سلامت روان"},
            {"step": "۰۲", "title": "تبیین پیامدهای فردی و بین‌فردی", "desc": items[1] if len(items) > 1 else "تخریب کارکردهای روانی-اجتماعی و افت چشمگیر بهزیستی ذهنی مراجعان"},
            {"step": "۰۳", "title": "محدودیت‌های مداخلات و پروتکل‌های سنتی", "desc": items[2] if len(items) > 2 else "نرخ بالای عود، عدم تعمیم‌پذیری طولانی‌مدت و فقدان انسجام ساختاری"},
            {"step": "۰۴", "title": "ضرورت طراحی و ارزیابی پروتکل هدفمند", "desc": items[3] if len(items) > 3 else "توسعه بسته مداخله‌ای بومی با تکیه بر سازوکارهای فراتشخیصی و تجربی"},
        ]
        steps_html = "".join(
            f'<div class="ad-step-card"><div class="ad-step-num">{_escape(s.get("step", ""))}</div><div class="ad-step-title">{_escape(s.get("title", ""))}</div><div class="ad-step-desc">{_escape(s.get("desc", ""))}</div></div>'
            for s in stages
        )
        return f"""
    <section class="slide ad-slide-problem {layout_class}" id="slide-{slide_number}" data-notes="{notes}" aria-label="{_escape(role)}" data-export-role="{_escape(role)}">
        <div class="ad-header">
            <span class="ad-eyebrow">فصل اول: تبیین مسئله و اهمیت پژوهش</span>
            <h2 class="ad-title">{title}</h2>
            <p class="ad-sub">{key_point}</p>
        </div>
        <div class="ad-funnel-steps">
            {steps_html}
        </div>
        {counter_html}
    </section>""".strip()

    # 4. Research Gap Matrix
    if layout_id == "defense_gap" or role in {"gap", "research_gap", "gap_matrix"}:
        col1_items = "".join(f'<li class="ad-gap-item">{_escape(it)}</li>' for it in items[:2]) or '<li class="ad-gap-item">تایید اثربخشی درمان‌های خط اول در متون خارجی</li>'
        col2_items = "".join(f'<li class="ad-gap-item">{_escape(it)}</li>' for it in items[2:4]) or '<li class="ad-gap-item">فقدان مطالعات پیگیری و خلأ آزمون همزمان متغیرهای واسطه‌ای</li>'
        col3_items = "".join(f'<li class="ad-gap-item">{_escape(ev)}</li>' for ev in evidence[:2]) or '<li class="ad-gap-item">ارزیابی اثربخشی پایدار با مدل‌سازی معادلات ساختاری و کنترل خطای نوع اول</li>'
        return f"""
    <section class="slide ad-slide-gap {layout_class}" id="slide-{slide_number}" data-notes="{notes}" aria-label="{_escape(role)}" data-export-role="{_escape(role)}">
        <div class="ad-header">
            <span class="ad-eyebrow">فصل اول: ضرورت، نوآوری و خلأ پژوهشی</span>
            <h2 class="ad-title">{title}</h2>
            <p class="ad-sub">{key_point}</p>
        </div>
        <div class="ad-gap-grid">
            <div class="ad-gap-card">
                <div class="ad-gap-card-title">دانسته نظری و پیشینه تجربی</div>
                <ul class="ad-gap-list">{col1_items}</ul>
            </div>
            <div class="ad-gap-card">
                <div class="ad-gap-card-title">شکاف و چالش در متون موجود</div>
                <ul class="ad-gap-list">{col2_items}</ul>
            </div>
            <div class="ad-gap-card">
                <div class="ad-gap-card-title">نوآوری و سهم علمی رساله</div>
                <ul class="ad-gap-list">{col3_items}</ul>
            </div>
        </div>
        {counter_html}
    </section>""".strip()
    ...
    # 5. Core Callout
    if layout_id == "defense_callout" or role in {"callout", "big_idea"}:
        quote = key_point or claim or title
        bullets_html = "".join(f'<li>{_escape(it)}</li>' for it in items[:3])
        return f"""
    <section class="slide ad-slide-callout {layout_class}" id="slide-{slide_number}" data-notes="{notes}" aria-label="{_escape(role)}" data-export-role="{_escape(role)}">
        <div class="ad-header">
            <span class="ad-eyebrow">گزاره بنیادین و فرضیه محوری رساله</span>
            <h2 class="ad-title">{title}</h2>
        </div>
        <div class="ad-callout-card">
            <div class="ad-callout-quote">{quote}</div>
            <ul class="ad-callout-bullets">{bullets_html}</ul>
        </div>
        {counter_html}
    </section>""".strip()

    # 6. Chapter Transition Divider
    if layout_id == "defense_divider" or role in {"divider", "section_divider"}:
        badge = _escape(spec.get("title_emphasis") or "گام پژوهش")
        sub_text = key_point or claim or "مرور ساختاریافته محتوای این بخش در چارچوب سوالات پژوهش"
        return f"""
    <section class="slide ad-slide-divider {layout_class}" id="slide-{slide_number}" data-notes="{notes}" aria-label="{_escape(role)}" data-export-role="{_escape(role)}">
        <div class="ad-divider-container">
            <div class="ad-divider-badge">{badge}</div>
            <h2 class="ad-divider-title">{title}</h2>
            <p class="ad-divider-sub">{sub_text}</p>
        </div>
        {counter_html}
    </section>""".strip()

    # 7. Conceptual Framework
    if layout_id == "defense_framework" or role in {"framework", "conceptual_model", "model"}:
        nodes = academic_data.get("nodes") or [
            {"title": "متغیر مستقل / مداخله", "desc": items[0] if len(items) > 0 else "مداخله روان‌شناختی مبتنی بر پذیرش و تعهد (ACT)"},
            {"title": "متغیر میانجی / سازوکار", "desc": items[1] if len(items) > 1 else "انعطاف‌پذیری روان‌شناختی و تنظیم شناختی هیجان"},
            {"title": "متغیر وابسته / پیامد", "desc": items[2] if len(items) > 2 else "کاهش نشانه‌های بالینی و ارتقای بهزیستی روان‌شناختی"},
        ]
        flow_html = ""
        for idx, node in enumerate(nodes):
            flow_html += f'<div class="ad-framework-node"><h4>{_escape(node.get("title", ""))}</h4><p>{_escape(node.get("desc", ""))}</p></div>'
            if idx < len(nodes) - 1:
                flow_html += '<div class="ad-framework-arrow">←</div>'
        return f"""
    <section class="slide ad-slide-framework {layout_class}" id="slide-{slide_number}" data-notes="{notes}" aria-label="{_escape(role)}" data-export-role="{_escape(role)}">
        <div class="ad-header">
            <span class="ad-eyebrow">فصل دوم: الگوی مفهومی و ساختار فرضیه‌ها</span>
            <h2 class="ad-title">{title}</h2>
            <p class="ad-sub">{key_point}</p>
        </div>
        <div class="ad-framework-flow">
            {flow_html}
        </div>
        {counter_html}
    </section>""".strip()
    ...
    # 8. Methodology
    if layout_id == "defense_methodology" or role in {"methodology", "research_design", "method", "sample_flow"}:
        steps = academic_data.get("method_cards") or [
            {"badge": "طرح پژوهش", "desc": items[0] if len(items) > 0 else "نیمه‌آزمایشی پیش‌آزمون-پس‌آزمون با گروه کنترل و پیگیری دوماهه"},
            {"badge": "جامعه و نمونه", "desc": items[1] if len(items) > 1 else "جامعه آماری مراجعان؛ N = ۳۰ نفر بر پایه تحلیل توان G*Power با گمارش تصادفی"},
            {"badge": "معیارهای ورود/خروج", "desc": items[2] if len(items) > 2 else "تشخیص بالینی قطعی، سن ۱۸ الی ۵۰ سال، عدم دریافت دارودرمانی موازی"},
            {"badge": "ابزار تحلیل داده‌ها", "desc": items[3] if len(items) > 3 else "آزمون کوواریانس چندمتغیری (MANCOVA) و اندازه اثر در نرم‌افزار SPSS 27"},
        ]
        cards_html = "".join(
            f'<div class="ad-method-card"><div class="ad-method-badge">{_escape(s.get("badge", ""))}</div><div class="ad-method-desc">{_escape(s.get("desc", ""))}</div></div>'
            for s in steps
        )
        return f"""
    <section class="slide ad-slide-methodology {layout_class}" id="slide-{slide_number}" data-notes="{notes}" aria-label="{_escape(role)}" data-export-role="{_escape(role)}">
        <div class="ad-header">
            <span class="ad-eyebrow">فصل سوم: روش‌شناسی و جامعه آماری</span>
            <h2 class="ad-title">{title}</h2>
            <p class="ad-sub">{key_point}</p>
        </div>
        <div class="ad-method-steps">
            {cards_html}
        </div>
        {counter_html}
    </section>""".strip()

    # 9. Psychometric Instruments
    if layout_id == "defense_instruments" or role in {"instruments", "instrument_matrix", "scales"}:
        rows = academic_data.get("scales") or [
            {"name": "پرسشنامه انعطاف‌پذیری روان‌شناختی (AAQ-II)", "items": "۱۰ گویه", "subscales": "پذیرش تجربی، عمل متعهدانه", "alpha": ".87"},
            {"name": "پرسشنامه تنظیم شناختی هیجان (CERQ)", "items": "۳۶ گویه", "subscales": "راهبردهای انطباقی و غیرانطباقی", "alpha": ".84"},
            {"name": "مقیاس بهزیستی روان‌شناختی ریف (RPWB)", "items": "۱۸ گویه", "subscales": "خودمختاری، تسلط بر محیط، رشد فردی", "alpha": ".89"},
        ]
        rows_html = "".join(
            f'<tr><td>{_escape(r.get("name", ""))}</td><td>{_escape(r.get("items", ""))}</td><td>{_escape(r.get("subscales", ""))}</td><td>{_escape(r.get("alpha", ""))}</td></tr>'
            for r in rows
        )
        return f"""
    <section class="slide ad-slide-instruments {layout_class}" id="slide-{slide_number}" data-notes="{notes}" aria-label="{_escape(role)}" data-export-role="{_escape(role)}">
        <div class="ad-header">
            <span class="ad-eyebrow">فصل سوم: ابزارهای پژوهش و شاخص‌های پایایی</span>
            <h2 class="ad-title">{title}</h2>
            <p class="ad-sub">{key_point}</p>
        </div>
        <div class="ad-table-wrap">
            <table class="ad-table">
                <thead><tr><th>ابزار سنجش</th><th>تعداد گویه</th><th>مولفه‌ها</th><th>آلفای کرونباخ (α)</th></tr></thead>
                <tbody>{rows_html}</tbody>
            </table>
            <div class="ad-table-note">* روایی صوری، محتوایی و سازه کلیه پرسشنامه‌ها در پژوهش حاضر به تایید رسید.</div>
        </div>
        {counter_html}
    </section>""".strip()

    # 10. Intervention Timeline
    if layout_id == "defense_intervention" or role in {"intervention", "intervention_timeline", "protocol"}:
        sessions = academic_data.get("sessions") or [
            {"num": "جلسه ۱-۲", "title": "مقدمه، قرارداد درمانی و ناامیدی خلاق", "tech": "تمایز بین درد پاک و درد ناپاک، استعاره چاله و بیل"},
            {"num": "جلسه ۳-۴", "title": "گسلش شناختی و حضور در لحظه اکنون", "tech": "تمرین تماشای افکار چون ابرها، تنفس متمرکز حسی"},
            {"num": "جلسه ۵-۶", "title": "خود به عنوان بافتار و شفاف‌سازی ارزش‌ها", "tech": "استعاره شطرنج و صفحه، شناسایی قطب‌نمای ارزش‌ها"},
            {"num": "جلسه ۷-۸", "title": "عمل متعهدانه، بازبینی و تثبیت دستاوردها", "tech": "هدف‌گذاری رفتاری SMART، طراحی پیشگیری از عود"},
        ]
        cards_html = "".join(
            f'<div class="ad-timeline-card"><div class="ad-timeline-badge">{_escape(s.get("num", ""))}</div><h4>{_escape(s.get("title", ""))}</h4><div class="ad-timeline-tech">{_escape(s.get("tech", ""))}</div></div>'
            for s in sessions
        )
        return f"""
    <section class="slide ad-slide-intervention {layout_class}" id="slide-{slide_number}" data-notes="{notes}" aria-label="{_escape(role)}" data-export-role="{_escape(role)}">
        <div class="ad-header">
            <span class="ad-eyebrow">فصل سوم: ساختار و سرفصل جلسات مداخله</span>
            <h2 class="ad-title">{title}</h2>
            <p class="ad-sub">{key_point}</p>
        </div>
        <div class="ad-timeline-steps">
            {cards_html}
        </div>
        {counter_html}
    </section>""".strip()
    ...
    if layout_id == "defense_descriptive" or role in {"descriptive", "descriptive_stats"}:
        rows = academic_data.get("descriptive_rows") or [
            {"var": "انعطاف‌پذیری روان‌شناختی (آزمایش)", "m_pre": "21.40 ± 3.82", "m_post": "34.60 ± 4.12", "skew": "-0.34", "kurt": "-0.28"},
            {"var": "انعطاف‌پذیری روان‌شناختی (کنترل)", "m_pre": "22.10 ± 3.95", "m_post": "22.80 ± 3.76", "skew": "0.41", "kurt": "-0.15"},
            {"var": "تنظیم هیجان مثبت (آزمایش)", "m_pre": "48.20 ± 6.15", "m_post": "65.40 ± 5.80", "skew": "-0.18", "kurt": "-0.52"},
            {"var": "تنظیم هیجان مثبت (کنترل)", "m_pre": "47.80 ± 5.90", "m_post": "48.10 ± 6.02", "skew": "0.22", "kurt": "-0.31"},
        ]
        rows_html = "".join(
            f'<tr><td>{_escape(r.get("var", ""))}</td><td>{_escape(r.get("m_pre", ""))}</td><td>{_escape(r.get("m_post", ""))}</td><td>{_escape(r.get("skew", ""))}</td><td>{_escape(r.get("kurt", ""))}</td></tr>'
            for r in rows
        )
        return f"""
    <section class="slide ad-slide-descriptive {layout_class}" id="slide-{slide_number}" data-notes="{notes}" aria-label="{_escape(role)}" data-export-role="{_escape(role)}">
        <div class="ad-header">
            <span class="ad-eyebrow">فصل چهارم: شاخص‌های توصیفی پیش‌آزمون و پس‌آزمون</span>
            <h2 class="ad-title">{title}</h2>
            <p class="ad-sub">{key_point}</p>
        </div>
        <div class="ad-table-wrap">
            <table class="ad-table">
                <thead><tr><th>متغیر و گروه</th><th>پیش‌آزمون (M ± SD)</th><th>پس‌آزمون (M ± SD)</th><th>چولگی</th><th>کشیدگی</th></tr></thead>
                <tbody>{rows_html}</tbody>
            </table>
            <div class="ad-table-note">توجه: شاخص‌های کشیدگی و چولگی در محدوده مجاز توزیع نرمال (۲±) قرار دارند.</div>
        </div>
        {counter_html}
    </section>""".strip()

    # 12. Split Comparison
    if layout_id == "defense_split" or role in {"split", "two_column", "comparison"}:
        col1_items = "".join(f'<li>{_escape(it)}</li>' for it in items[:3]) or '<li>رشد معنادار در تمامی خرده‌مقیاس‌ها</li><li>تثبیت دستاوردها در مرحله پیگیری دوماهه</li>'
        col2_items = "".join(f'<li>{_escape(it)}</li>' for it in items[3:6]) or '<li>تغییرات غیرمعنادار نمرات در طول زمان</li><li>ماندگاری الگوهای ناسازگارانه پیشین</li>'
        return f"""
    <section class="slide ad-slide-split {layout_class}" id="slide-{slide_number}" data-notes="{notes}" aria-label="{_escape(role)}" data-export-role="{_escape(role)}">
        <div class="ad-header">
            <span class="ad-eyebrow">فصل چهارم: تحلیل مقایسه‌ای عملکرد گروه‌ها</span>
            <h2 class="ad-title">{title}</h2>
            <p class="ad-sub">{key_point}</p>
        </div>
        <div class="ad-split-container">
            <div class="ad-split-col">
                <h3>گروه مداخله (درمان تجربی)</h3>
                <ul>{col1_items}</ul>
            </div>
            <div class="ad-split-col">
                <h3>گروه کنترل (لیست انتظار)</h3>
                <ul>{col2_items}</ul>
            </div>
        </div>
        {counter_html}
    ...
    # 13. Hero Spotlight
    if layout_id == "defense_spotlight" or role in {"spotlight", "result_spotlight", "metrics"}:
        num_facts = spec.get("numeric_facts") or []
        hero_stat = _escape(num_facts[0] if num_facts else "F = 34.82, p < .001, η² = .48")
        caption = key_point or claim or "تاثیر چشمگیر و معنادار مداخله بر متغیرهای پیامد در سطح خطای کمتر از یک‌هزارم"
        metrics = [
            ("F-Statistic", "F(1, 27) = 34.82"),
            ("Significance", "p < .001"),
            ("Effect Size", "Partial η² = .48"),
            ("Observed Power", "1 - β = 0.99"),
        ]
        pills_html = "".join(f'<div class="ad-stat-pill"><strong>{_escape(k)}:</strong> {_escape(v)}</div>' for k, v in metrics)
        return f"""
    <section class="slide ad-slide-spotlight {layout_class}" id="slide-{slide_number}" data-notes="{notes}" aria-label="{_escape(role)}" data-export-role="{_escape(role)}">
        <div class="ad-header">
            <span class="ad-eyebrow">فصل چهارم: شاخص محوری و اندازه اثر برجسته</span>
            <h2 class="ad-title">{title}</h2>
        </div>
        <div class="ad-stat-box">
            <div class="ad-stat-hero">{hero_stat}</div>
            <p class="ad-stat-caption">{caption}</p>
            <div class="ad-stat-metrics">{pills_html}</div>
        </div>
        {counter_html}
    </section>""".strip()

    # 14. Inferential Statistics Table (ANCOVA)
    if layout_id == "defense_table" or role in {"table", "stat_table", "ancova", "evidence"}:
        rows = academic_data.get("ancova_rows") or [
            {"source": "پیش‌آزمون (کوواریت)", "ss": "142.30", "df": "1", "ms": "142.30", "f": "18.45", "p": ".001", "eta": ".41"},
            {"source": "گروه (اثر اصلی درمان)", "ss": "268.50", "df": "1", "ms": "268.50", "f": "34.82", "p": "< .001", "eta": ".48"},
            {"source": "خطا (Error)", "ss": "208.20", "df": "27", "ms": "7.71", "f": "-", "p": "-", "eta": "-"},
            {"source": "کل (Total)", "ss": "619.00", "df": "29", "ms": "-", "f": "-", "p": "-", "eta": "-"},
        ]
        rows_html = "".join(
            f'<tr><td>{_escape(r.get("source", ""))}</td><td>{_escape(r.get("ss", ""))}</td><td>{_escape(r.get("df", ""))}</td><td>{_escape(r.get("ms", ""))}</td><td>{_escape(r.get("f", ""))}</td><td>{_escape(r.get("p", ""))}</td><td>{_escape(r.get("eta", ""))}</td></tr>'
            for r in rows
        )
        return f"""
    <section class="slide ad-slide-table {layout_class}" id="slide-{slide_number}" data-notes="{notes}" aria-label="{_escape(role)}" data-export-role="{_escape(role)}">
        <div class="ad-header">
            <span class="ad-eyebrow">فصل چهارم: جدول تحلیل کوواریانس تک‌متغیری (ANCOVA)</span>
            <h2 class="ad-title">{title}</h2>
            <p class="ad-sub">{key_point}</p>
        </div>
        <div class="ad-table-wrap">
            <table class="ad-table">
                <thead><tr><th>منبع تغییرات</th><th>مجموع مجذورات (SS)</th><th>درجه آزادی (df)</th><th>میانگین مجذورات (MS)</th><th>آماره F</th><th>سطح معناداری (p)</th><th>اندازه اثر (η²)</th></tr></thead>
                <tbody>{rows_html}</tbody>
            </table>
            <div class="ad-table-note">اثر پیش‌آزمون به عنوان متغیر همپراش آماری تعدیل شده است. معنادار در p < .001.</div>
        </div>
        {counter_html}
    </section>""".strip()

    # 15. Hypothesis Verification Matrix
    if layout_id == "defense_matrix" or role in {"matrix", "hypothesis_matrix", "hypothesis", "proof", "verification"}:
        hypotheses = academic_data.get("hypotheses") or [
            {"title": "فرضیه ۱: بهبود انعطاف‌پذیری روان‌شناختی", "stat": "F = 34.82, p < .001", "verdict": "تأیید شد", "pass": True},
            {"title": "فرضیه ۲: ارتقای راهبردهای انطباقی هیجان", "stat": "F = 28.14, p < .001", "verdict": "تأیید شد", "pass": True},
            {"title": "فرضیه ۳: کاهش درماندگی روان‌شناختی مراجعان", "stat": "F = 19.75, p = .001", "verdict": "تأیید شد", "pass": True},
            {"title": "فرضیه ۴: پایداری نتایج در مرحله پیگیری", "stat": "t = 1.12, p = .274 (بدون افت)", "verdict": "تأیید شد", "pass": True},
        ]
        cards_html = "".join(
            f'<div class="ad-matrix-card"><div class="ad-matrix-card-header"><h4>{_escape(h.get("title", ""))}</h4><span class="ad-badge-pass">{_escape(h.get("verdict", ""))}</span></div><div class="ad-matrix-stat">{_escape(h.get("stat", ""))}</div></div>'
            for h in hypotheses
        )
        return f"""
    <section class="slide ad-slide-matrix {layout_class}" id="slide-{slide_number}" data-notes="{notes}" aria-label="{_escape(role)}" data-export-role="{_escape(role)}">
        <div class="ad-header">
            <span class="ad-eyebrow">فصل چهارم: جمع‌بندی کارنامه آزمون فرضیه‌ها</span>
            <h2 class="ad-title">{title}</h2>
            <p class="ad-sub">{key_point}</p>
        </div>
        <div class="ad-matrix-grid">
            {cards_html}
        </div>
        {counter_html}
    </section>""".strip()

    # 16. Discussion Mechanisms
    if layout_id == "defense_discussion" or role in {"discussion", "discussion_mechanism", "mechanism"}:
        cards = academic_data.get("mechanisms") or [
            {"theory": "مدل فراتشخیصی پذیرش و تعهد (Hayes et al.)", "mech": items[0] if len(items) > 0 else "افزایش گسلش شناختی سبب کاهش همجوشی مراجع با افکار آزاردهنده و ارتقای تاب‌آوری می‌گردد."},
            {"theory": "نظریه فرآیندی تنظیم هیجان (Gross)", "mech": items[1] if len(items) > 1 else "تغییر کانون توجه و ارزیابی مجدد شناختی پیش از برانگیختگی هیجانی افراطی مانع رفتارهای تکانشی می‌شود."},
            {"theory": "همسویی با پژوهش‌های داخلی و بین‌المللی", "mech": items[2] if len(items) > 2 else "یافته‌های این رساله همسو با پژوهش‌های خط اول در ایران و جهان، روایی بومی پروتکل را تایید می‌کند."},
        ]
        cards_html = "".join(
            f'<div class="ad-discuss-card"><div class="ad-discuss-theory">{_escape(c.get("theory", ""))}</div><div class="ad-discuss-mech">{_escape(c.get("mech", ""))}</div></div>'
            for c in cards
        )
        return f"""
    <section class="slide ad-slide-discussion {layout_class}" id="slide-{slide_number}" data-notes="{notes}" aria-label="{_escape(role)}" data-export-role="{_escape(role)}">
        <div class="ad-header">
            <span class="ad-eyebrow">فصل پنجم: تبیین و سازوکارهای نظری یافته‌ها</span>
            <h2 class="ad-title">{title}</h2>
            <p class="ad-sub">{key_point}</p>
        </div>
        <div class="ad-discuss-grid">
            {cards_html}
        </div>
        {counter_html}
    </section>""".strip()
    ...
    if layout_id == "defense_implications" or role in {"implications"}:
        cards = academic_data.get("implications") or [
            {"domain": "کاربردهای بالینی و درمانی", "text": items[0] if len(items) > 0 else "به‌کارگیری پروتکل حاضر به عنوان بسته مداخله‌ای اثربخش در کلینیک‌های روان‌شناسی و مراکز مشاوره"},
            {"domain": "سیاست‌گذاری سلامت روان", "text": items[1] if len(items) > 1 else "آموزش مهارت‌های تنظیم هیجان و انعطاف‌پذیری روانی در قالب کارگاه‌های پیشگیرانه سازمانی"},
            {"domain": "دلالت‌های آموزشی و پژوهشی", "text": items[2] if len(items) > 2 else "توسعه واحدهای درسی تخصصی و پژوهش‌های چندمرکزی در سطح ملی"},
        ]
        cards_html = "".join(
            f'<div class="ad-imp-card"><div class="ad-imp-domain">{_escape(c.get("domain", ""))}</div><div class="ad-imp-text">{_escape(c.get("text", ""))}</div></div>'
            for c in cards
        )
        return f"""
    <section class="slide ad-slide-implications {layout_class}" id="slide-{slide_number}" data-notes="{notes}" aria-label="{_escape(role)}" data-export-role="{_escape(role)}">
        <div class="ad-header">
            <span class="ad-eyebrow">فصل پنجم: دلالت‌های کاربردی و بالینی</span>
            <h2 class="ad-title">{title}</h2>
            <p class="ad-sub">{key_point}</p>
        </div>
        <div class="ad-imp-grid">
            {cards_html}
        </div>
        {counter_html}
    </section>""".strip()

    # 18. Limitations & Recommendations
    if layout_id == "defense_limitations" or role in {"limitations", "recommendations"}:
        cards = academic_data.get("limitations") or [
            {"badge": "محدودیت روش‌شناختی", "text": items[0] if len(items) > 0 else "تکیه بر پرسشنامه‌های خودگزارش‌دهی و احتمال سوگیری همسویی آزمودنی‌ها"},
            {"badge": "محدودیت نمونه‌گیری", "text": items[1] if len(items) > 1 else "محدود بودن جامعه آماری به مراجعان یک شهر و لزوم احتیاط در تعمیم به سایر مناطق"},
            {"badge": "پیشنهاد پژوهشی آتی", "text": items[2] if len(items) > 2 else "ارزیابی اثربخشی در ترکیب با شاخص‌های فیزیولوژیک (مانند تغییرپذیری ضربان قلب HRV)"},
        ]
        cards_html = "".join(
            f'<div class="ad-lim-card"><div class="ad-lim-badge">{_escape(c.get("badge", ""))}</div><div class="ad-lim-text">{_escape(c.get("text", ""))}</div></div>'
            for c in cards
        )
        return f"""
    <section class="slide ad-slide-limitations {layout_class}" id="slide-{slide_number}" data-notes="{notes}" aria-label="{_escape(role)}" data-export-role="{_escape(role)}">
        <div class="ad-header">
            <span class="ad-eyebrow">فصل پنجم: محدودیت‌های پژوهش و افق‌های آینده</span>
            <h2 class="ad-title">{title}</h2>
            <p class="ad-sub">{key_point}</p>
        </div>
        <div class="ad-lim-grid">
            {cards_html}
        </div>
        {counter_html}
    </section>""".strip()

    # 19. Closing & Viva Voce Q&A
    if layout_id == "defense_closing" or role in {"closing", "cta", "cta_close"}:
        quote = key_point or "«پایان جلسه ارائه دفاعیه رساله — آماده استماع نقطه نظرات، راهنمایی‌ها و پاسخگویی به پرسش‌های استادان ارجمند»"
        return f"""
    <section class="slide ad-slide-closing {layout_class}" id="slide-{slide_number}" data-notes="{notes}" aria-label="{_escape(role)}" data-export-role="{_escape(role)}">
        <div class="ad-closing-container">
            <div class="ad-closing-thanks">با سپاس و احترام از توجه استادان و داوران ارجمند</div>
            <div class="ad-closing-quote">{_escape(quote)}</div>
            <div class="ad-closing-meta">{title}</div>
        </div>
        {counter_html}
    </section>""".strip()
    ...
    # Default fallback
    cards_html = "".join(f'<div class="ad-card"><h4>{_escape(it)}</h4></div>' for it in items[:4])
    return f"""
    <section class="slide {layout_class}" id="slide-{slide_number}" data-notes="{notes}" aria-label="{_escape(role)}" data-export-role="{_escape(layout_id)}">
        <div class="ad-header">
            <h2 class="ad-title">{title}</h2>
            <p class="ad-sub">{key_point}</p>
        </div>
...
def _render_custom_theme_slide(
    ...
    *,
    ...
    preset_norm = _normalize_preset_name(style_contract.get("preset", ""))
    if preset_norm in {
        "academic defense",
        "academic-defense",
        "academic navy",
    }:
        return _render_academic_defense_slide(spec, total, role_index=role_index)
