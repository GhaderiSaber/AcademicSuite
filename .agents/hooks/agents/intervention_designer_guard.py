...
# -*- coding: utf-8 -*-
"""
.agents/hooks/agents/intervention_designer_guard.py
Dedicated Lifecycle Hook Guard for intervention-designer.
Enforces:
1. Directive 12: Worker delegation guard (blocks invoke_subagent).
2. Capability Boundary: Blocks run_command (protocol designer does not execute shell scripts).
...
def handle_pre_tool_use(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if name in ('invoke_subagent', 'define_subagent'):
        return {'decision': 'deny', 'reason': 'CONSTITUTIONAL VIOLATION (Directive 12): intervention-designer cannot spawn subagents.'}
    if name == 'run_command':
        return {'decision': 'deny', 'reason': 'CONSTITUTIONAL VIOLATION (Capability Boundary): intervention-designer designs protocols and manuals; shell execution is revoked.'}
