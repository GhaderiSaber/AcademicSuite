...
4. Directive 4: Strict APA 7 precision and Persian leading zero standard.
"""
...
def handle_pre_tool_use(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if name in ('invoke_subagent', 'define_subagent'):
        return {'decision': 'deny', 'reason': 'CONSTITUTIONAL VIOLATION (Directive 12): longitudinal-modmed-expert cannot spawn subagents.'}
...
def handle_stop(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        if re.search(r'\bp\s*=\s*\.?000\b', last_resp, re.I) or "p = .000" in last_resp or "p = 0.000" in last_resp or re.search(r'[pP]\s*=\s*۰\.۰۰۰', last_resp):
            return {'decision': 'continue', 'reason': 'CONSTITUTIONAL VIOLATION (Directive 4): longitudinal-modmed-expert must report p < .001. Reporting p = .000 is strictly forbidden.'}
        if re.search(r'(?<![۰-۹0-9])\.[۰-۹]+', last_resp):
            return {'decision': 'continue', 'reason': 'CONSTITUTIONAL VIOLATION (Directive 4): Detected naked Persian decimal. You must preserve the leading zero.'}
    except Exception:
        pass
    return {'decision': 'allow'}
