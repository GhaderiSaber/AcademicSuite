...
def is_shortcut_command(cmd: str) -> Tuple[bool, str]:
    """
    Directive 25: Universal Anti-Shortcut & Zero-Fastpath Shell Guard
    Blocks shell commands explicitly attempting to bypass validation, omit tests, or skip safety gates.
    """
    if not cmd:
        return False, ""
    bypass_flags = [
    ...
    for flag_pat in bypass_flags:
        if re.search(flag_pat, cmd, re.IGNORECASE):
            return True, (
                f"CONSTITUTIONAL VIOLATION (Directive 25 - Universal Anti-Shortcut & Zero-Fastpath Invariant): "
                f"Execution bypass flag detected in command ('{cmd}'). "
                f"Zero permission for fastpaths, shortpaths, or validation bypasses. "
                f"All tasks, tests, and validation gates must be executed properly and thoroughly without haste."
            )
    return False, ""
...
def is_dangerous_command(cmd: str) -> Tuple[bool, str]:
    ...
    is_shortcut_violation, shortcut_reason = is_shortcut_command(cmd)
