...
- **Dedicated CLI Entrypoint:** `scripts/academic_docgen.py` with sealed subcommands (`render-docx`, `render-markdown`, `scaffold-triad`, `compile-thesis`, `compile-presentation`, `scaffold-apa-tables`, `polish-tone`).
- **Permitted Execution Scope:** Bundled document scripts in declared writing skills (`chapter-4-writing`, `persian-thesis-builder`, `persian-defense-presentation-builder`, `apa-reporting`, `ai-academic-tone-polisher`, etc.) and shared document utilities (`scripts/academic_docgen.py`, `scripts/structured_docx_generator.py`, `scripts/persian_docx_engine.py`, `scaffold_chapter4_triad.py`, `pandoc`, `soffice`, `mkdir`, `cp`).
