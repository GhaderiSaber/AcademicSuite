...
# -*- coding: utf-8 -*-
"""
.agents/hooks/agents/knowledge_curator_guard.py
Dedicated Lifecycle Hook Guard for knowledge-curator.
Enforces:
1. Directive 12: Worker delegation guard (blocks invoke_subagent).
2. Scoped Knowledge Invariant: Writes allowed strictly to .agents/learning/ or .agents/memory/.
...
def handle_pre_tool_use(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if name in ('invoke_subagent', 'define_subagent'):
        return {'decision': 'deny', 'reason': 'CONSTITUTIONAL VIOLATION (Directive 12): knowledge-curator cannot spawn subagents.'}
