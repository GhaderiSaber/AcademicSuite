...
hooks:
  - .agents/hooks/agents/statistical_expert_hook.json
---
...
2. **Directive 2 (Deterministic Calculations)**: Never calculate statistics, p-values, or effect sizes mentally. As a statistical modeling planner, never execute computational scripts directly; always delegate execution to deterministic CLI scripts ("The Hands") executed by specialist workers (`statistics-agent`) via `invoke_subagent`.
