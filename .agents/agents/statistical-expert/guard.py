...
def handle_pre_tool_use(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if name in ('invoke_subagent', 'define_subagent'):
        return {'decision': 'deny', 'reason': 'CONSTITUTIONAL VIOLATION (Directive 12): statistical-expert cannot spawn subagents.'}
    if name in ('write_to_file', 'replace_file_content', 'multi_replace_file_content', 'apply_diff', 'edit_file'):
        return {'decision': 'deny', 'reason': 'CONSTITUTIONAL VIOLATION (Advisor Read-Only): statistical-expert cannot mutate workspace files.'}
    if name == 'run_command':
        return {'decision': 'deny', 'reason': 'CONSTITUTIONAL VIOLATION (Execution Revocation): statistical-expert cannot run terminal commands directly.'}
