...
class TestJournalSubmissionWorkflow(unittest.TestCase):
    ...
    def test_01_spec_file_exists_and_valid(self):
        """Verifies that journal_submission.md exists in legacy archive and modern skill exists."""
        spec_path = os.path.join(WORKFLOWS_DIR, "journal_submission.md")
        if not os.path.exists(spec_path):
            spec_path = os.path.join(LEGACY_WORKFLOWS_DIR, "journal_submission.md.bak")
        self.assertTrue(os.path.exists(spec_path), f"Missing workflow specification: {spec_path}")

        # Also verify modern skill exists
        skill_path = os.path.join(AGENTS_DIR, "skills", "journal-submission-assistant", "SKILL.md")
